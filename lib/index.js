...
const transpileCode = async ({ code }) => {
  ...
  const handlers = {
    TSAsExpression: ({ node }) => {
      ...
      const endOfExpressionToEnd = code.substring(node.expression.range[1], node.range[1]);

      const indexOfAs = endOfExpressionToEnd.indexOf("as");
      if (indexOfAs < 0) {
        throw Error("BUG: as not found");
      }

      const start = node.expression.range[1] + indexOfAs;
    ...
    TSEnumDeclaration: ({ node }) => {
      ...
      node.members.forEach((member, idx) => {
        ...
        let rawInitializer = undefined;

        // console.log({ member });

        if (member.initializer) {

          rawInitializer = code.substring(member.initializer.range[0], member.initializer.range[1]);
          ...
          rawInitializer = `(${lastInitializer.raw}) + ${idx - lastInitializer.index}`;
        }

        initializers.push(`${member.id.name}: ${rawInitializer}`);
      });
